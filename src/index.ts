...
async function modelValidation(e:Env,networkType='win'){
  const [tc,mc,rc,invalid,dated] = await Promise.all([
    ...
    e.DB.prepare('SELECT COUNT(*) n FROM matches').first<any>(),
    e.DB.prepare('SELECT COUNT(*) n FROM rankings').first<any>(),
  ...
  if(networkType==='win'){
    const x=await e.DB.prepare('SELECT COUNT(*) n FROM matches WHERE home_score<>away_score AND home_team_id<>away_team_id').first<any>();
...
async function triggerGithub(e:Env,runId:number){
  ...
  for(const mode of modes){
    ...
    if(mode==='workflow_dispatch'){
      ...
      body={event_type:'run-ergm',client_payload:{run_id:String(runId),base_url:e.PUBLIC_BASE_URL||''}};
    }
...
async function systemHealth(e:Env){
  ...
  try{
    ...
    const counts=await e.DB.prepare('SELECT (SELECT COUNT(*) FROM teams) teams,(SELECT COUNT(*) FROM matches) matches,(SELECT COUNT(*) FROM rankings) rankings').first<any>();
