...
const STEP_LABELS_KO:Record<string,string>={
  'Report GitHub run ID to API':'실행 ID 보고',
  'Checkout repository':'저장소 체크아웃',
  'Setup R':'R 환경 설정',
  'Install and cache R dependencies':'R 패키지 설치',
...
async function githubRunProgress(e:Env,modelRunId:number){
  ...
  if(!run)return{ok:false,error:'model run not found'};
  if(!run.github_run_id)return{ok:true,phase:run.status,steps:[],note:'GitHub 실행이 아직 시작되지 않았거나 보고되지 않았습니다.'};
  const owner=(e.GITHUB_OWNER||'').trim(),repo=(e.GITHUB_REPO||'').trim();
  if(!owner||!repo)return{ok:false,error:'GITHUB_OWNER/GITHUB_REPO 설정이 필요합니다.'};
  try{
...
function buildInterpretPrompt(model:any):string{
  const coefs=Array.isArray(model.coefficients)&&model.coefficients.length
    ? model.coefficients.map((c:any)=>`- ${c.term}: estimate=${Number(c.estimate).toFixed(4)}, SE=${Number(c.std_error).toFixed(4)}, p=${Number(c.p_value).toFixed(4)}, odds_ratio=${Number(c.odds_ratio).toFixed(3)}`).join('\n')
    : '(계수 정보 없음)';
  return `당신은 네트워크 통계학(ERGM) 전문가입니다. 아래는 FIFA 국가대표 경기 네트워크에 대한 ERGM(지수족 랜덤 그래프 모형) 적합 결과입니다. 연구자가 논문 초안에 참고할 수 있도록 한국어로 해설해 주세요. 확정적 인과주장 대신 통계적으로 신중한 표현을 쓰십시오.

[모형 정보]
- 모형명: ${model.model_name||model.name||'-'}
- 네트워크 종류: ${model.network_type||'-'}
- 사용된 공식: ${model.formula||'-'}${model.fallback_used?` (원래 요청식 "${model.requested_formula}"이 수렴하지 않아 자동으로 단순화됨)`:''}
- 노드 수: ${model.node_count??'-'}, 엣지 수: ${model.edge_count??'-'}
- 데이터 규모: 팀 ${model.input_counts?.teams??'-'}개, 경기 ${model.input_counts?.matches??'-'}건, 랭킹 기록 ${model.input_counts?.rankings??'-'}건
- 수렴 여부: ${model.converged?'수렴':'미수렴 또는 확인 필요'}
- AIC: ${model.aic??'-'}, BIC: ${model.bic??'-'}

[계수 추정치]
${coefs}

다음 JSON 형식으로만 답하세요. 다른 설명, 마크다운, 코드블록 없이 순수 JSON 객체 하나만 출력하세요:
...
async function aiInterpretation(e:Env,model:any){
  ...
  return{
    ...
    coefficientNotes:Array.isArray(parsed.coefficient_notes)?parsed.coefficient_notes.slice(0,20).map((x:any)=>String(x).slice(0,300)):[],
    cautions:(Array.isArray(parsed.cautions)&&parsed.cautions.length?parsed.cautions:['계수의 방향·유의확률·MCMC 및 GOF를 함께 확인하십시오.']).slice(0,10).map((x:any)=>String(x).slice(0,300)),
    aiGenerated:true,
    aiModel:modelId
  }
}
...
if(p==='/api/model-comparison')return j(await modelComparison(e));if(p==='/api/reproducibility')return j(await reproducibilityPackage(e));if(p==='/api/interpret'){
  ...
  if(!x)return j({...deterministicInterpretation({}),aiGenerated:false});
  if(x.status!=='completed')return j({...deterministicInterpretation(x),aiGenerated:false,runId:x.id});
  let resultJson:any={};try{resultJson=JSON.parse(x.result_json||'{}')}catch{}
  const merged={...x,...resultJson,converged:resultJson.converged??x.converged};
  try{
...
if(p==='/api/models'&&r.method==='POST'){if(!auth(r,e))return j({error:'unauthorized'},401);const b=await r.json() as any,networkType=b.networkType||'win',validation=await modelValidation(e,networkType),analysisMode=(['ergm','tergm','both'].includes(b.analysisMode)?b.analysisMode:'both') as 'both'|'ergm'|'tergm';if(!validation.ok)return j({ok:false,status:'validation_failed',error:'ERGM 실행 전 데이터 검증에 실패했습니다.',validation},422);
  // v37: mutual(상호성)은 방향 네트워크(win)에서만 의미가 있습니다. 프런트엔드에서
  // match(무방향) 선택 시 mutual 체크박스를 자동으로 비활성화하지만, API를 직접 호출하는
  // 경우까지 대비해 서버에서도 한 번 더 걸러냅니다. 이전에는 이걸 안 걸러서 "요청한 공식"과
  // "R이 실제로 적합한 공식"이 달라지는 혼란(GitHub 재현성 리포트의 formula 필드 불일치)이
  // 있었습니다.
  let formula=b.formula||'network ~ edges', mutualStripped=false;
  if(networkType==='match'&&/\bmutual\b/.test(formula)){
    formula=formula.replace(/\s*\+\s*mutual\b/,'').replace(/\bmutual\b\s*\+\s*/,'').trim();
    if(/^network\s*~\s*mutual\s*$/.test(formula))formula='network ~ edges';
    mutualStripped=true;
  }
  const x=await e.DB.prepare(`INSERT INTO model_runs(model_name,network_type,formula,status,diagnostics_json) VALUES(?,?,?,'queued',?) RETURNING id`).bind(b.name||'ERGM',networkType,formula,JSON.stringify({preflight:validation,analysisMode,mutualStripped})).first<any>();if(!x?.id)return j({error:'모형 요청 저장 실패'},500);try{const dispatch=await triggerGithub(e,x.id,analysisMode);await e.DB.prepare(`UPDATE model_runs SET status='dispatched' WHERE id=?`).bind(x.id).run();return j({ok:true,runId:x.id,status:'dispatched',analysisMode,note:`사전검증을 통과하여 GitHub Actions R 분석(${analysisMode})을 요청했습니다.${mutualStripped?' (경기 네트워크는 무방향이라 mutual 항을 자동으로 제외했습니다.)':''}`,dispatch,validation})}catch(err){await e.DB.prepare(`UPDATE model_runs SET status='dispatch_failed',diagnostics_json=? WHERE id=?`).bind(JSON.stringify({preflight:validation,analysisMode,error:String(err)}),x.id).run();return j({ok:false,runId:x.id,status:'dispatch_failed',error:err instanceof Error?err.message:String(err),validation},502)}}
